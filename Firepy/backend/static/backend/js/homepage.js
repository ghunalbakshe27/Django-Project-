...
(function () {
    ...
    function playSong(idx) {
        ...
        // 🔥 Save the state of song 
        saveMiniPlayerState(song, idx);
    ...
    mpAudio.addEventListener('timeupdate', () => {
        ...
        // 🔥 On every 2 second save the time 
        if (Math.floor(mpAudio.currentTime) % 2 === 0) {
            localStorage.setItem('mp_currentTime', mpAudio.currentTime);
        }
    });
    ...
    // ── Save state ──
    function saveMiniPlayerState(song, idx) {
        localStorage.setItem('mp_state', JSON.stringify({
            idx: idx,
            song: song,          // poora song object save karo
            currentTime: 0       // fresh start, timeupdate update karega
        }));
    }
    ...
    function restoreMiniPlayer() {
        const raw = localStorage.getItem('mp_state');
        if (!raw) return;                          // kuch saved nahi — kuch mat karo

        try {
            const state = JSON.parse(raw);
            const song = state.song;
            if (!song || !song.audio_url) return;  // corrupt state — skip

            // Queue mein wapas dalo taaki prev/next kaam kare
            queue = [song];
            currentIdx = 0;
            ...
            // Audio set karo — play mat karo abhi
            mpAudio.src = song.audio_url;

            // Saved time pe seek karo jab audio ready ho
            const savedTime = parseFloat(localStorage.getItem('mp_currentTime') || '0');

            mpAudio.addEventListener('loadedmetadata', () => {
                if (savedTime > 0 && savedTime < mpAudio.duration) {
                    mpAudio.currentTime = savedTime;

                    // Progress bar bhi dikhao
                    const pct = (savedTime / mpAudio.duration) * 100;
                    mpProgressFill.style.width = pct + '%';
                    mpProgressThumb.style.left = pct + '%';
                    mpCurrentTime.textContent = fmtTime(savedTime);
                    mpDuration.textContent = fmtTime(mpAudio.duration);
                }
            }, { once: true });

            // Player dikhao — paused state mein
            miniPlayer.classList.add('show');
            setPlayState(false);   // paused — user khud play kare

        } catch (e) {
            clearMiniPlayerState();  // corrupt data — saaf karo
        }
    }

    // ── Close player mein clear bhi karo ──
    // (purana mpClose listener replace karo)
    mpClose.addEventListener('click', () => {
        mpAudio.pause();
        miniPlayer.classList.remove('show');
        setPlayState(false);
        currentIdx = -1;
        clearMiniPlayerState();   // 🔥 localStorage saaf karo
    });

    // ── Page load pe restore karo ──
    restoreMiniPlayer();
